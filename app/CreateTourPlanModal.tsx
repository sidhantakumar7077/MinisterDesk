// screens/CreateTourPlanPage.tsx
...
type Props = {
  mode: 'create' | 'edit';
  onClose?: () => void; // e.g. navigation.goBack()
...
export default function CreateTourPlanPage({
  mode,
  onClose,
  onSubmit,
  initial,
  ...
  const buildInitialForm = (seed?: Partial<TourPlanForm>): TourPlanForm => {
    const safeCategory =
      (seed?.category && categories.includes(seed.category)
        ? seed.category
        : categories[0]) || categories[0];
    const safeTravelMode = (seed?.travelMode as TourPlanForm['travelMode']) ?? 'Car';
    return {
      title: seed?.title ?? '',
  ...
  // keyboard tracking (used to dynamically set ScrollView padding so there is no persistent gap)
  ...
  const getTravelIcon = (mode: string, active = false) => {
    ...
    switch (mode) {
      ...
      case 'Train':
        return <Train size={16} color={color} />;
      case 'Car':
  ...
  // unified submit handler: calls parent onSubmit if present; otherwise inserts to supabase and navigates back
  const handleSubmit = async () => {
    if (submitting) return;
    if (!validate()) {
      // scroll to top of errors if needed (we keep it simple)
      return;
    }
    ...
    try {
      if (onSubmit) {
        await onSubmit(form);
        Alert.alert('Success', mode === 'create' ? 'Tour created.' : 'Tour updated.');
        if (onClose) onClose();
        else router.back();
      } else {
      ...
      Alert.alert('Error', e?.message ?? 'Failed to save tour plan');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  // used by Back button and Cancel
  const handleCancel = () => {
    if (onClose) onClose();
    else router.back();
  };
        ...
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          <View style={{ flex: 1 }}>
            ...
            <ScrollView
              ...
              contentContainerStyle={{ paddingBottom: Math.max(insets.bottom, keyboardHeight + 20) }}
              ...
              <View style={styles.formSection}>
                ...
                <View style={styles.scheduleContainer}>
                  ...
                  <View style={styles.scheduleCard}>
                    ...
                    {/* Start Date */}
                    <TouchableOpacity style={[styles.dateSelector, errors.startDate && styles.inputError]} onPress={() => openPicker('startDate')}>
                      <View style={styles.dateSelectorContent}>
                        <Calendar size={16} color="#6b7280" />
                        <Text style={[styles.dateSelectorText, !form.startDate && styles.placeholderText]}>{fmtDate(form.startDate)}</Text>
                      </View>
                    </TouchableOpacity>
                    ...
                    <View style={styles.timeInputWrapper}>
                      ...
                      <TouchableOpacity style={{ flex: 1 }} onPress={() => openPicker('startTime')}>
                        <Text style={[styles.customTimeInput, !form.startTime && styles.placeholderText]}>{fmtTime(form.startTime)}</Text>
                      </TouchableOpacity>
                    </View>
                  ...
                  <View style={styles.scheduleCard}>
                    ...
                    {/* End Date */}
                    <TouchableOpacity style={[styles.dateSelector, errors.endDate && styles.inputError]} onPress={() => openPicker('endDate')}>
                      <View style={styles.dateSelectorContent}>
                        <Calendar size={16} color="#6b7280" />
                        <Text style={[styles.dateSelectorText, !form.endDate && styles.placeholderText]}>{fmtDate(form.endDate)}</Text>
                      </View>
                    </TouchableOpacity>
                    ...
                    <View style={styles.timeInputWrapper}>
                      ...
                      <TouchableOpacity style={{ flex: 1 }} onPress={() => openPicker('endTime')}>
                        <Text style={[styles.customTimeInput, !form.endTime && styles.placeholderText]}>{fmtTime(form.endTime)}</Text>
                      </TouchableOpacity>
                    </View>
                ...
                <View style={styles.inputGroup}>
                  ...
                  <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.optionsScroll}>
                    {travelModes.map((mode) => {
                      ...
                      return (
                        <TouchableOpacity key={mode} style={[styles.optionChip, active && styles.selectedOptionChip]} onPress={() => updateField('travelMode', mode)}>
                ...
                <View style={styles.inputGroup}>
                  ...
                  <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.optionsScroll}>
                    {categories.map((c) => {
                      ...
                      return (
                        <TouchableOpacity key={c} style={[styles.categoryChip, active && styles.selectedCategoryChip]} onPress={() => updateField('category', c)}>
            ...
            {/* footer */}
            <View style={[styles.modalFooter, { paddingBottom: insets.bottom ? insets.bottom : 12 }]}>
              <TouchableOpacity style={styles.cancelButton} onPress={handleCancel} disabled={submitting || !!loading}>
                <Text style={styles.cancelButtonText}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.submitButton, (submitting || !!loading) && { opacity: 0.7 }]}
                onPress={handleSubmit}
                disabled={submitting || !!loading}
              >
                <Save size={16} color="#ffffff" />
                <Text style={styles.submitButtonText}>{submitting ? 'Saving...' : submitLabel ?? (mode === 'edit' ? 'Save Changes' : 'Create Tour Plan')}</Text>
              </TouchableOpacity>
            </View>
